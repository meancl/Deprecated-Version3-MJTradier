namespace MJTradier
{
    public partial class Form1
    {
        ...
        // 천장선 : 0% ~ 60% 까지 있음
        public double[] arrCeiling = new double[STEP_TRADE] { 0, 0.005, 0.01, 0.015, 0.02, 0.025, 0.03, 0.035, 0.04, 0.045, 0.05,
        ...
        // 기본형 바닥선 : 처음 2퍼 차이, 1프로는 1.5프로 차이, 1.5프로부터는 1프로 차이
        public double[] arrFloor = new double[STEP_TRADE] { -0.02, -0.015, -0.005, 0.005, 0.01, 0.015, 0.02, 0.025, 0.03,
                                                                ...
                                                                0.485, 0.49, 0.495, 0.5, 0.505,  0.51, 0.515, 0.52, 0.525, 0.53, 0.535,
                                                                0.54, 0.545, 0.55, 0.555, 0.56, 0.565, 0.57, 0.575, 0.58, 0.585, 0.6
                                                           };

        // 자비로운 바닥선 : 초기 바닥라인을 천장선과 차이가 많이 남
        public double[] arrRoughFloor = new double[STEP_TRADE] { -0.06, -0.04, -0.035, 0.01, 0.015, 0.015, 0.02, 0.025, 0.025, 0.03, 0.03,
                                                                0.035, 0.04, 0.045, 0.05, 0.055, 0.06, 0.065, 0.07, 0.075, 0.08, 0.085,
                                                                0.09, 0.095, 0.1, 0.105, 0.11, 0.115, 0.12, 0.125, 0.13, 0.135, 0.14,
                                                                0.145, 0.15,  0.155, 0.16, 0.165, 0.17, 0.175, 0.18, 0.185, 0.19, 0.195,
                                                                0.2, 0.205, 0.21, 0.215, 0.22, 0.225, 0.23, 0.235, 0.24, 0.245, 0.25,
        ...
        /// <summary>
        /// 천장선의 해당인덱스의 값을 반환해줌.
        /// 마지막 인덱스의 이상의 인덱스값이 들어오면 마지막 인덱스로 재설정하고 마지막 인덱스값으로 반환해줌. 
        /// </summary>
        /// <param name="fCurTargetIdx"></param>
        /// <returns></returns>
        double GetHigherCeilingTarget(ref int fCurTargetIdx)
        {
        ...
        /// <summary>
        /// 기본형 바닥선의 해당인덱스의 값을 반환해줌
        /// 마지막 인덱스의 이상의 인덱스값이 들어오면 마지막 인덱스로 재설정하고 마지막 인덱스값으로 반환해줌. 
        /// </summary>
        /// <param name="fCurTargetIdx"></param>
        /// <returns></returns>
        double GetHigherFloorTarget(ref int fCurTargetIdx)
        {
            if (fCurTargetIdx >= STEP_TRADE)
                fCurTargetIdx = STEP_TRADE - 1;
            return arrFloor[fCurTargetIdx];
        }
    }
}
